...
Passwordless.prototype.acceptToken = function(options) {
	...
	return async function(ctx, next) {
		...
		if(!token && !uid && options.allowPost) {
			if(!req.body) {
				throw new Error('req.body does not exist: did you require middleware to accept POST data (such as koa-bodyparser) before calling acceptToken?')
		...
		if(token && uid) {
			self._tokenStore.authenticate(token, uid.toString(), async function(error, valid, referrer) {
				if(valid) {
					...
					if(!self._allowTokenReuse) {
						self._tokenStore.invalidateUser(uid, function(err) {
							if(err) {
								await next('TokenStore.invalidateUser() error: ' + error);
							} else {
								success();
							}
						})
					...
					await next('TokenStore.authenticate() error: ' + error);
...
Passwordless.prototype.restricted = function(options) {
	...
	return async function(ctx, next) {
		...
		if(options.failureRedirect) {
			...
			self._send401(ctx, 'Provide a token');
  		}
	}
}
	...
	return async function(ctx, next) {
		...
		var sendError = function(statusCode, authenticate) {
			if(options.failureRedirect) {
				...
				if(statusCode === 401) {
					self._send401(ctx, authenticate)
				} else {
		...
		getUserID(user, delivery, async function(uidError, uid) {
			if(uidError) {
				await next(new Error('Error on the user verification layer: ' + uidError));
			} else if(uid) {
				var token;
				try {
					...
					await next(new Error('Error while generating a token: ' + err));
				}
					...
					if(storeError) {
						...
						deliveryMethod.sendToken(token, uid, user, function(deliveryError) {
							if(deliveryError) {
								...
								ctx.passwordless.uidToAuth = uid;
								if(options.successFlash) {
									req.flash('passwordless-success', options.successFlash);
								}
								await next();
 ...
 * @private
 */
Passwordless.prototype._send401 = function(ctx, authenticate) {
	ctx.status = 401
	if(authenticate) {
		ctx.set({ 'WWW-Authenticate': authenticate });
	}
	ctx.body = {
		error: 'Unauthorized',
		success: false,
